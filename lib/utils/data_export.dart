  ...
  static String generateStatisticsSummary(List<Car> cars) {
    ...
    if (soldCars.isNotEmpty) {
      final totalPrice = soldCars.fold<double>(
        0,
        (sum, car) => sum + (double.tryParse(car.price.replaceAll(',', '')) ?? 0),
      );
    ...
    buffer.writeln('Satış Oranı: ${totalCars > 0 ? ((soldCars.length / totalCars) * 100).toStringAsFixed(1) : '0'}%');
    buffer.writeln();
    
    if (soldCars.isNotEmpty) {
      buffer.writeln('SATIŞ İSTATISTIKLERİ');
      buffer.writeln('-' * 20);
      buffer.writeln('Ortalama Satış Fiyatı: ${averagePrice.toStringAsFixed(0)} TL');
      buffer.writeln();
    }
    ...
    if (brandCount.isNotEmpty) {
      ...
      for (final entry in sortedBrands) {
        final percentage = totalCars > 0 ? (entry.value / totalCars * 100).toStringAsFixed(1) : '0';
    ...
    if (fuelTypeCount.isNotEmpty) {
      ...
      for (final entry in sortedFuelTypes) {
        final percentage = soldCars.isNotEmpty ? (entry.value / soldCars.length * 100).toStringAsFixed(1) : '0';
        buffer.writeln('${entry.key}: ${entry.value} araç (%$percentage)');
      }
    }
