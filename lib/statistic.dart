...
class _StatisticPageState extends State<StatisticPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      logger.info('Loading statistics data from database', tag: 'Statistics');
      ...
      logger.info('Successfully loaded statistics data',
          tag: 'Statistics',
          data: {
      ...
      setState(() {
        isLoading = false;
        errorMessage =
            'İstatistik verileri yüklenirken hata oluştu: ${e.toString()}';
      });

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('İstatistik verileri yüklenirken hata oluştu'),
      ...
      return const Center(
        child: Column(
          ...
          children: [
            ...
            SizedBox(height: 16),
            Text('İstatistik verileri yükleniyor...'),
          ],
        ),
      );
    }
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'İstatistik analizi için araç verileri gerekli',
  ...
  Widget _buildOverviewCards() {
    ...
    return GridView.count(
      ...
      children: [
        ...
        _buildOverviewCard(
          'Satış Oranı',
          '${cars.isNotEmpty ? (soldCars / totalCars * 100).toStringAsFixed(1) : '0'}%',
          Icons.trending_up,
          Colors.indigo,
        ),
      ],
  ...
  Widget _buildBrandItem(Map<String, dynamic> brand) {
    final soldCars = cars.where((car) => car.isSold);
    final totalSold = soldCars.length;
    final percentage =
        soldCars.isNotEmpty ? (brand['count'] / totalSold * 100).round() : 0;
  ...
  Widget _buildPriceRangeItem(Map<String, dynamic> range) {
    final filteredCars = _getFilteredCars();
    final totalSold = filteredCars.length;
    final percentage = filteredCars.isNotEmpty
        ? (range['count'] / totalSold * 100).round()
        : 0;
